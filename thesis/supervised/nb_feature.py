...
#            words.remove(item)
    if match in words:
          ...
          for i in range(ind+1, min(ind+4, len(words))):
                j = i-ind
          ...
          for i in range(max(0, ind-3), ind):
                j = ind-i
                #features['left-context-word-%s' % j]= words[i][0]
                features['left-context-tag-%s(%s)' % (j, tags[i])] = True

          for i in range(ind+1, min(ind+4, len(words))):
                j = i-ind
              #  features['right-context-word-%s' % j]= words[i][0]
                features['right-context-tag-%s(%s)' % (j, tags[i])] = True#          if ind<len(words)-1:
        ...
        # print name
                ...
                for line in file:
                      if len(line.split())>2:
                             ...
                             feat_set.append((bigram_feature(words),sense))
